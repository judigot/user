...
main() {    
    ...
    codeToBeRemoved=(".tsx")
    removeTextContent "codeToBeRemoved[@]"
    local strictPackages=("eslint-plugin-import" "eslint-import-resolver-typescript" "globals" "@eslint/js" "typescript-eslint" "eslint" "eslint-config-prettier" "eslint-plugin-prettier" "eslint-plugin-react" "eslint-plugin-react-hooks" "eslint-plugin-react-refresh" "eslint-plugin-no-type-assertion")
...
modifyESLintConfig() {
    ...
    # Hardcoded ESLint config with Next.js auto-detection and test globals
    cat > "$fileName" << 'ESLINT_EOF'
